...
def get_submission_info(username):
    ...
    while True:
        submissions = json.loads(requests.get(
            f'https://kenkoooo.com/atcoder/atcoder-api/v3/user/submissions?user={username}&from_second={cur}').text)
        if not submissions:
            break
...
def get_solutions(username, all_info=None):
    ...
    responses = grequests.imap(grequests.get(
        info['link'], headers=headers) for info in all_info)

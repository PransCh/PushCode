...
def get_submission_info(username):
    submissions = json.loads(requests.get(
        f'https://codeforces.com/api/user.status?handle={username}').text)['result']
...
def get_code(driver):
    lines = driver.find_elements(
        By.CSS_SELECTOR, '#program-source-text > ol > li')
...
def get_solutions(username, all_info=None):
    ...
    try:
        pages = int(driver.find_elements(By.CSS_SELECTOR,
                    '#pageContent > div > ul > li > span')[-1].text)
    ...
    for sub_id in sub_ids:
        ...
        if fail_counter:
            try:
                ...
                if fail_counter == 0:
                    driver.get(
                        f'https://codeforces.com/submissions/{username}/page/{index}')
        ...
        for _ in range(3):
            try:
                element = driver.find_element(
                    By.PARTIAL_LINK_TEXT, str(sub_id))
                ...
                while not cur or ((cur in prev) and ((
                    ...
                    cur = '\n'.join(ele.text for ele in driver.find_elements(
                        By.CSS_SELECTOR, '#facebox > div > div > div > pre > code > ol > li'))
